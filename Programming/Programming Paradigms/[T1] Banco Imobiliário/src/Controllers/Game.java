    ...
    public void loadGame() {
        ...
        if (savedState != null) {
            ...
            synchronized (this) {
                new Thread(() -> {
                    while (true) {
                        Player currentPlayer = gameManager.getGameBoard().getPlayers().get(i[0]);
                        gameBoardView.displayPlayerInfo(currentPlayer);
    
                        saveGame(); // Salva automaticamente ao final de cada turno
                        // Se o jogador falir, indica que ele perdeu e o remove do jogo
                        if (gameBoard.getPlayers().get(i[0]).getMoney() <= 0) {
                            gameBoard.getPlayers().remove(i[0]);
                            gameBoardView.removePlayer(gameBoard.getPlayers().get(i[0]));
