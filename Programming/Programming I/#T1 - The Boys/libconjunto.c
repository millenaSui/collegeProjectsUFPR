...
int retira_cjt(conjunto_t *c, int elemento) {
    ...
    int indice;
    int init = 0;
    int tam = cardinalidade_cjt(c) - 1;
    while (tam >= init) {
        int meio = (tam + init / 2);
        if ((*c).v[meio] == elemento)
            indice = meio;
        if ((*c).v[meio] < elemento)
            init = meio + 1;
        else
    ...
    for (i = indice; i < cardinalidade_cjt(c) - 1; i++) {
        (*c).v[i] = (*c).v[i + 1];
    }
    (*c).card--;
    return 1;
...
conjunto_t *uniao_cjt(conjunto_t *c1, conjunto_t *c2) {
    ...
    while ((*c1).ptr <= cardinalidade_cjt(c1) && (*c2).ptr <= cardinalidade_cjt(c2))
    {
        ...
        if ((*c1).v[(*c1).ptr] < (*c2).v[(*c2).ptr] || (*c2).ptr == cardinalidade_cjt(c2)) {
            insere_cjt(cjt_uniao, (*c1).v[(*c1).ptr]);
            (*c1).ptr++;

            /* Se elemento de C2 é menor que elemento de C1
               Insere elemento de C2 no vetor uniao*/
        } else if ((*c1).v[(*c1).ptr] >= (*c2).v[(*c2).ptr] || (*c1).ptr == cardinalidade_cjt(c1)) {
